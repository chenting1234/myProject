<template>
    <div class="app-container">
        <!-- 数据表格 -->
        <el-table :data="messageList" style="width: 100%" border v-loading="listLoading" element-loading-text="Loading"
            ...
            <el-table-column prop="content" label="评论内容" align="center" width="150">
            </el-table-column>
                ...
                <template slot-scope="{ row,  }">
                    <el-tooltip class="item" effect="dark" content="删除" placement="top" :hide-after="3000">
                        <el-button type="danger" icon="el-icon-delete" circle size="mini"
                            @click="deleteMessage(row)"></el-button>
                    </el-tooltip>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        async fetchData() {
            ...
            const resp = await this.$API.message.getMessage(this.currPage, this.pageSize);
        ...
        deleteMessage(messageInfo) {
            this.$confirm('删除该评论, 是否继续?', '是否删除该评论', {
                ...
                this.$API.message.deleteMessage(messageInfo.id).then(resp => {
                    this.$message({
                        type: 'success',
                        message: '删除成功!'
                    });
